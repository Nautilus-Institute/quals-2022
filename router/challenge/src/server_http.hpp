  ...
  public:
    ...
    class Response : public std::enable_shared_from_this<Response>, public std::ostream {
      ...
      void write_header(const CaseInsensitiveMultimap &header, size_type size) {
        ...
        for(auto &field : header) {
          ...
          else if (!has_content_type && case_insensitive_equal(field.first, "content-type"))
            has_content_type = true;
          else if (!has_server && case_insensitive_equal(field.first, "server"))
            has_server = true;
        ...
        if (!has_server)
          *this << "Server: nginx/1.14.0 (Ubuntu)\r\n";
    ...
    public:
      ...
      void redirect(std::string target) {
        ...
        header.emplace("Location", target);
        write(StatusCode::redirection_found, std::string(), header);
      }

      void redirect(std::string target, const CaseInsensitiveMultimap &header) {
        CaseInsensitiveMultimap header_copy(header);
        header_copy.emplace("Location", target);
        write(StatusCode::redirection_found, std::string(), header_copy);
      }
    ...
    void read(const std::shared_ptr<Session> &session) {
      ...
      asio::async_read_until(*session->connection->socket, session->request->streambuf, "\r\n\r\n", [this, session](const error_code &ec, std::size_t bytes_transferred) {
        ...
        if(!ec) {
          ...
          // If cookies, load username and password
          session->request->username = "";
          session->request->password = "";
          auto cookie_it = session->request->header.find("Cookie");
          if (cookie_it != session->request->header.end()) {
            auto username_it = cookie_it->second.find("username=");
            auto password_it = cookie_it->second.find("password=");
            if (username_it != std::string::npos && password_it != std::string::npos) {
              session->request->username = "admin";
              session->request->password = "password";
            }
          }
